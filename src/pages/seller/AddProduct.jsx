...
const AddProduct = () => {
   ...
   return (
      <div className="no-scrollbar overflow-y-scroll h-[calc(100vh-61px)] flex grow flex-col justify-between  ">
         <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-lg">
               ...
               <div className="flex items-center gap-3 mt-2">
                  {Array(4).fill('').map((_, index) => (
                     <label key={index} htmlFor={`image${index}`}>
                        ...
                        <img src={files[index] ? URL.createObjectURL(files[index]) : assets.upload_area} alt="image" width={90} />
                     </label>
                  ))}
               </div>
            </div>
            ...
            <button disabled={loading} className="px-8 py-2.5 bg-indigo-500 text-white font-medium rounded cursor-pointer">
               {
                  loading ? <PiSpinnerGap className="text-2xl mx-auto animate-spin" /> : <span className="uppercase ">ADD</span>
               }
            </button>
         </form>
      </div>
